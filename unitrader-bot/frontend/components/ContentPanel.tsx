  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div>
        ...
        <p className="page-subtitle">Generate and manage blog and social content</p>
      </div>

      <div className="flex gap-1 rounded-xl bg-dark-900/50 border border-dark-800/50 p-1 text-sm">
        {(["blog", "social"] as SubTab[]).map((t) => (
          <button
            key={t}
            onClick={() => setSubTab(t)}
            className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-medium transition-all ${
              subTab === t ? "bg-[#0d1117] text-white shadow-sm" : "text-dark-400 hover:text-dark-200"
            }`}
          >
            {t === "blog" ? <FileText size={13} /> : <Share2 size={13} />}
      ...
      {loading ? (
        ...
        blogPosts.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-dark-500">No blog posts yet. Generate your first one above.</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {blogPosts.map((post) => (
              <div key={post.id} className="rounded-2xl border border-dark-800 bg-[#0d1117] p-4 hover:border-dark-700 transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate text-sm font-medium text-white">{post.title}</h3>
                    <div className="mt-1.5 flex items-center gap-3 text-xs text-dark-500">
                  ...
                  {post.status === "draft" && (
                    <button
                      ...
                      className="flex shrink-0 items-center gap-1.5 rounded-lg border border-brand-500/20 px-3 py-1.5 text-xs text-brand-400 transition-colors hover:bg-brand-500/10 disabled:opacity-50"
                    >
        ...
        socialPosts.length === 0 ? (
          <div className="py-16 text-center">
            <Share2 size={32} className="mx-auto mb-3 text-dark-600" />
            <p className="text-sm text-dark-500">No social posts yet. Generate some above.</p>
          </div>
        ) : (
          <div className="space-y-2.5">
              ...
              return (
                <div key={post.id} className="rounded-2xl border border-dark-800 bg-[#0d1117] p-4 hover:border-dark-700 transition-colors">
                  <div className="mb-2.5 flex items-center gap-2">
                    <PlatformIcon size={13} className={color} />
                    ...
                    <span className="ml-auto text-[11px] text-dark-600">{new Date(post.created_at).toLocaleDateString()}</span>
                  </div>
